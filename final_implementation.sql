create DATABASE University_HR_ManagementSystem_128;
GO
USE University_HR_ManagementSystem_128;
GO
...
-- Fix the View to include Department
CREATE OR ALTER VIEW v_HR_All_Leaves AS
SELECT 
    L.request_ID, L.date_of_request, L.start_date, L.end_date, L.num_days, L.final_approval_status,
    CASE 
        ...
        ELSE 'Unknown'
    END AS leave_type,
    E.first_name + ' ' + E.last_name AS Applicant,
    E.dept_name AS Department -- <--- THIS PREVENTS THE CRASH
FROM Leave L
...
LEFT JOIN Employee E ON E.employee_ID = COALESCE(al.emp_ID, acl.emp_ID, ml.Emp_ID, ul.Emp_ID, cl.emp_ID);
GO

-- New function to retrieve ALL attendance records for a given employee (regardless of month/year)
IF OBJECT_ID('MyAttendance_All', 'IF') IS NOT NULL
    DROP FUNCTION MyAttendance_All;
GO
CREATE FUNCTION MyAttendance_All(@employee_ID int)
RETURNS TABLE
AS
RETURN (
    SELECT *
...
-- New function to retrieve ALL payroll records for a given employee (regardless of month)
CREATE OR ALTER FUNCTION AllPayroll(@employee_ID int)
RETURNS TABLE
AS
RETURN (
    SELECT *
